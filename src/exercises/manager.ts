...
export default class ExercisesManager {
    static async createExercise(
        exercise: Partial<ExerciseType>): Promise<ExerciseType> {
        try {

            const response = await ExercisesRepository.createExercise(exercise);
            await setToCache('exercises', response._id, JSON.stringify(response), 3600);
    ...
    static async getResultByUserAndExerciseId(exerciseId: string, useId: string): Promise<ResultType | undefined | null> {
        try {
            const cachedResult = await getFromCache('getResultByUserAndExerciseId', exerciseId + useId);
            if (cachedResult) {
                console.log("Cache hit: exercises manager - getResultByUserAndExerciseId", exerciseId, useId);
                return JSON.parse(cachedResult); // Parse cached JSON data
            }
            console.log("exercises manager getResultByUserAndExerciseId - exerciseId", exerciseId);
            const result = await ExercisesRepository.getResultByUserAndExerciseId(exerciseId, useId);
            result !== null ? await setToCache('getResultByUserAndExerciseId', exerciseId + useId, JSON.stringify(result), 36000) : null;
            console.log("exercises manager getResultByUserAndExerciseId - result", result);
            return result;
        } catch (error: any) {
            console.error('Manager Error [getResultByUserAndExerciseId]:', error.message);
            throw new Error('Error in getResultByUserAndExerciseId');
        }
    }

    static async getRelevantByExerciseId(exerciseId: string): Promise<TargetType[] | null> {
        try {
            const cachedRelevant = await getFromCache('getRelevantByExerciseId', exerciseId);
            if (cachedRelevant) {
                console.log("Cache hit: exercises manager - getRelevantByExerciseId", exerciseId);
                return JSON.parse(cachedRelevant); // Parse cached JSON data
            }
            console.log("Exercise manager getRelevantByExerciseId - exerciseId", exerciseId);
            const relevant = await ExercisesRepository.getRelevantByExerciseId(exerciseId);
            relevant !== null ? await setToCache('getRelevantByExerciseId', exerciseId, JSON.stringify(relevant), 36000) : null;

            console.log("Exercise manager getRelevantByExerciseId - relevant targets", relevant);
            return relevant;
        } catch (error: any) {
    ...
    static async getAnswersByExerciseId(exerciseId: string): Promise<TargetType[] | undefined | null> {
        try {
            ...
            if (cachedTargets) {
                console.log("Cache hit: Exercise manager - getAnswersByExerciseId", exerciseId);
                return JSON.parse(cachedTargets); // Parse cached JSON data
            }
            console.log("Exercise manager getAnswersByExerciseId - exerciseId", exerciseId);
            const targets = await ExercisesRepository.getAnswersByExerciseId(exerciseId);
            targets !== null ? await setToCache('getAnswersByExerciseId', exerciseId, JSON.stringify(targets), 36000) : null;
    ...
    static async getExerciseById(exerciseId: string): Promise<ExerciseType | null> {
        try {
            const cachedExercise = await getFromCache('getExerciseById', exerciseId);
            if (cachedExercise) {
                console.log("Cache hit: Exercise manager - getExerciseById", exerciseId);
    ...
    static async getAllExercise(): Promise<ExerciseType[]> {
        try {
            const cachedExercises = await getFromCache('getAllExercise', 'allExercises');
            if (cachedExercises) {
                console.log("Cache hit: Exercise manager - getAllExercise", cachedExercises);
                return JSON.parse(cachedExercises); // Parse cached JSON data
            }
            const exercises = await ExercisesRepository.getAllExercises();
            await setToCache('getAllExercise', 'allExercises', JSON.stringify(exercises), 3600);
            return exercises;
        } catch (error: any) {
    ...
    static async updateExercise(
        ...
        try {
            ...
            await setToCache('exercises', exerciseId, JSON.stringify(updatedExercise), 3600);
