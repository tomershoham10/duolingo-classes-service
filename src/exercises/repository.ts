...
export default class ExercisesRepository {
    static async createExercise(exercise: Partial<ExerciseType>): Promise<ExerciseType> {
        try {
    ...
    static async getResultByUserAndExerciseId(exerciseId: string, userId: string): Promise<ResultType | null> {
        try {
            const exercise = await ExerciseModel.findById(exerciseId);
            console.log("exercises repo getResultByUserAndExerciseId - exercise", exercise);
            if (exercise && userId) {
                const result = await ResultsModel.findOne({ exerciseId: { $eq: exerciseId }, userId: { $eq: userId } });
                console.log("exercises repo getResultByUserAndExerciseId - result", result);
                return result;
            ...
            console.error('Repository Error:', error.message);
            throw new Error(`exercises repo - getResultByUserAndExerciseId: ${error}`);
        }
    }
        ...
        try {
            ...
            if (exercise && exercise.type === ExercisesTypes.FSA) {
                ...
                if (relevantIds) {
                    ...
                    console.log("exercises repo getRelevantByExerciseId - targetsDetails", relevantIds, targetsDetails);
