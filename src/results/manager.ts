...
export default class ResultsManager {
    ...
    static async getResultById(resultId: string): Promise<ResultType | null> {
        try {
            ...
            result !== null ? await setToCache('results', resultId, JSON.stringify(result), 3600) : null;
    ...
    static async getResultsByUserId(userId: string): Promise<ResultType[] | null> {
        try {
            ...
            await setToCache('getResultsByUserId', userId, JSON.stringify(results), 3600);
    ...
    static async getResultsByLessonAndUser(lessonId: string, userId: string): Promise<ResultType[] | null> {
        try {
            const cachedResults = await getFromCache('getResultsByLessonAndUser', lessonId + userId);
            if (cachedResults) {
                console.log("Cache hit: results manager - getResultsByLessonAndUser", lessonId, userId);
                return JSON.parse(cachedResults); // Parse cached JSON data
            }
            const results = await ResultsRepository.getResultsByLessonAndUser(lessonId, userId);
            await setToCache('getResultsByLessonAndUser', lessonId + userId, JSON.stringify(results), 3600);
    ...
    static async getAllResults(): Promise<ResultType[]> {
        try {
            ...
            if (cachedResults) {
                console.log("Cache hit: results manager - getAllResults", cachedResults);
                return JSON.parse(cachedResults); // Parse cached JSON data
            }


            const results = await ResultsRepository.getAllResults();
            await setToCache('getAllResults', 'allResults', JSON.stringify(results), 3600);
    ...
    static async updateResult(
        ...
        try {
            ...
            await setToCache('results', resultId, JSON.stringify(updatedResult), 3600);
            await resetNamespaceCache('getAllResults', 'allResults');
