...
export default class LessonsManager {
    ...
    static async getsExercisesByLessonId(lessonId: string): Promise<FSAType[]> {
        try {
            ...
            if (cachedExercises) {
                console.log("Cache hit: lessons manager - getsExercisesByLessonId", lessonId);
            ...
            await setToCache('getsExercisesByLessonId', lessonId, JSON.stringify(exercises), 3600)
    ...
    static async getsUnsuspendedExercisesByLessonId(lessonId: string): Promise<FSAType[]> {
        try {
            ...
            if (cachedExercises) {
                console.log("Cache hit: lessons manager - getsUnsuspendedExercisesByLessonId", lessonId);
                return JSON.parse(cachedExercises); // Parse cached JSON data
            }

            const exercises = await LessonsRepository.getsUnsuspendedExercisesByLessonId(lessonId);
            await setToCache('getsUnsuspendedExercisesByLessonId', lessonId, JSON.stringify(exercises), 3600)
    ...
    static async getResultsByLessonIdAndUserId(lessonId: string, userId: string): Promise<{ numOfExercises: number, results: ResultType[] } | null | undefined> {
        try {
            const cachedResults = await getFromCache('getResultsByLessonIdAndUserId', lessonId + userId);
            if (cachedResults) {
                console.log("Cache hit: lessons manager - getResultsByLessonIdAndUserId", lessonId + userId);
            ...
            await setToCache('getResultsByLessonIdAndUserId', lessonId + userId, JSON.stringify(results), 3600)
    ...
    static async getAllLessons(): Promise<LessonsType[]> {
        try {
            ...
            await setToCache('getAllLessons', 'allLessons', JSON.stringify(lessons), 3600);
    ...
    static async updateLesson(
        ...
        try {
            ...
            await setToCache('lessons', lessonId, JSON.stringify(updatedLesson), 3600);
            await resetNamespaceCache('getAllLessons', 'allLessons');
