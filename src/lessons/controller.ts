...
//     static async createByLevel(req: Request, res: Response) {    try {      const levelId = req.params.levelId;      const session = await mongoose.startSession();      session.startTransaction();      console.log('controller - createByLevel: levelId ', levelId);      const level = await LevelsModel.findById(levelId);      console.log('controller - createByLevel: level ', level);      if (!level) {        await session.abortTransaction();        session.endSession();        return res.status(404).json({ message: 'level not found' });      }      // Create a new exercise directly instead of a lesson      const exercisesModel = (await import('../exercises/model.js')).default;      const newExercise = await exercisesModel.create({        type: 'fsa', // Default type        dateCreated: new Date(),        adminComments: 'Created via level'      });      // Add exercise directly to the level instead of creating a lesson      level.exercisesIds        ? level.exercisesIds.push(newExercise._id.toString())        : (level.exercisesIds = [newExercise._id.toString()]);            await level.save({ session: session });      await session.commitTransaction();      session.endSession();            res        .status(201)        .json({           message: 'New exercise created and level updated successfully',          exerciseId: newExercise._id        });    } catch (error: any) {      console.error('Controller Error:', error.message);      res.status(400).json({ error: error.message });    }  }
